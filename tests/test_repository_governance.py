...
class RepositoryGovernanceTest(unittest.TestCase):
    ...
    def test_agents_declares_miniapp_side_rules(self):
        agents = self.read_text("AGENTS.md")

        self.assertIn("HotKey 跨仓通用规范", agents)
        self.assertIn("hotkey-server 是跨仓库 AGENTS.md 主规范源", agents)
        self.assertIn("Taro + React + TypeScript", agents)
        self.assertIn("@umijs/openapi", agents)
        self.assertIn("小程序不得手写后端 API 类型", agents)
        self.assertIn("server -> web -> miniapp -> 回归", agents)

    def test_readme_declares_miniapp_scope_and_validation_command(self):
        readme = self.read_text("README.md")

        self.assertIn("# hotkey-miniapp", readme)
        self.assertIn("Taro 跨端小程序", readme)
