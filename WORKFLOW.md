# WORKFLOW.md — hotkey-miniapp 工作流调度契约

本文件定义 hotkey-miniapp 项目基于 Linear Ticket 的工作流调度规范。
...
本工作流默认使用 Codex 执行，也可以通过 Linear 标签 `agent:codex`、`agent:claude`、`agent:cursor` 覆盖使用的 Agent。所有进度事实源和可复用流程路径应与当前 Agent 保持一致。
