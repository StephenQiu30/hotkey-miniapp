...
This workflow uses Claude as the default implementation runtime. It uses `.claude/`
skill paths and the `## Claude Workpad` marker because those are the installed
project workflow assets.
...
16. For `Todo` tickets that already had a PR attached at kickoff:
    ...
    - Then move to `Agent Review` (applying the same reviewer label logic as step 15).
