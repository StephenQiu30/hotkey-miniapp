...
- This workflow is scoped to `hotkey-server`.
- Do not modify `hotkey-web` or `hotkey-miniapp` unless the Linear issue explicitly names those repositories.
...
- Run `go test ./...` before handoff when Go code changed.
...
5. Run validation/tests required for the scope.
   ...
   - You may make temporary local proof edits to validate assumptions (for example: tweak a local build input for `make`, or hardcode a response path) when this increases confidence.
