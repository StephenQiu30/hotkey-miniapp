...
purpose: "将 `内容标准化与去重` PRD 拆成可执行任务、验证命令和回滚边界。"
canonical_path: "docs/plans/5-内容标准化与去重实现计划.md"
status: approved
version: "1.0.0"
owner: "StephenQiu30"
inputs:
...
outputs:
  ...
  - 内容标准化与去重验证证据
triggers:
  - "docs/product/prd/5-内容标准化与去重PRD.md 变更"
...
将来源内容统一为 SourceItem，并完成 URL、hash、标题和时间窗口去重。

## 2. 文件清单

- PRD：`docs/product/prd/5-内容标准化与去重PRD.md`
- Plan：`docs/plans/5-内容标准化与去重实现计划.md`
...
- 原始 URL、来源、发布时间、抓取时间、hash 和元数据可追踪。
