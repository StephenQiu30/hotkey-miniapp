...
canonical_path: "docs/plans/20-秒级实时检测实现计划.md"
status: approved
version: "1.0.0"
owner: "StephenQiu30"
inputs:
  - docs/product/prd/20-秒级实时检测PRD.md
  - docs/engineering/1-Go后端重建与开源仓库治理设计.md
outputs:
  - 秒级实时检测实现任务
  - 秒级实时检测验证证据
triggers:
  - "docs/product/prd/20-秒级实时检测PRD.md 变更"
...
- PRD：`docs/product/prd/20-秒级实时检测PRD.md`
- Plan：`docs/plans/20-秒级实时检测实现计划.md`
