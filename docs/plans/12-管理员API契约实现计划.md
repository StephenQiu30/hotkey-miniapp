...
purpose: "将 `管理员API契约` PRD 拆成可执行任务、验证命令和回滚边界。"
canonical_path: "docs/plans/12-管理员API契约实现计划.md"
status: approved
version: "1.0.0"
owner: "StephenQiu30"
inputs:
  - docs/product/prd/12-管理员API契约PRD.md
  - docs/engineering/1-Go后端重建与开源仓库治理设计.md
outputs:
  - 管理员API契约实现任务
  - 管理员API契约验证证据
triggers:
  - "docs/product/prd/12-管理员API契约PRD.md 变更"
...
定义管理员后台 API，覆盖关键词、来源、任务、事件审查和日报生成。

## 2. 文件清单

- PRD：`docs/product/prd/12-管理员API契约PRD.md`
- Plan：`docs/plans/12-管理员API契约实现计划.md`
...
- 管理员可启停来源和关键词，可查询任务运行记录，可触发日报生成。
